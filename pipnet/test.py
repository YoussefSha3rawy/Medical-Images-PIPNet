...
def eval_pipnet(net,
    ...
    for i, (xs, ys) in test_iter:
        ...
        with torch.no_grad():
            ...
            almost_nz = torch.count_nonzero(torch.gt(torch.abs(pooled),
                                                     1e-3).float(),
                                            dim=1).float()
    ...
    info['local_size_all_classes'] = local_size_total / len(
        test_loader.dataset)
    ...
    if net.module._num_classes == 2:
        ...
        try:
            for classname, classidx in test_loader.dataset.class_to_idx.items(
                ...
                if classidx == 0:
                    print("Accuracy positive class (", classname, classidx,
                          ") (TPR, Sensitivity):", tp / (tp + fn))
                elif classidx == 1:
                    print("Accuracy negative class (", classname, classidx,
...
def get_thresholds(net,
                   ...
                   progress_prefix: str = 'Get Thresholds Epoch') -> dict:
