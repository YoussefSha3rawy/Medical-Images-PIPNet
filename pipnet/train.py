        ...
        if not pretrain:
            with torch.no_grad():
                ...
                net._classification.normalization_multiplier.copy_(
                    torch.clamp(
                        net._classification.normalization_multiplier.data,
                        min=1.0))
                if net._classification.bias is not None:
                    net._classification.bias.copy_(
...
def calculate_loss(proto_features,
                   pooled,
                   out,
                   ys1,
                   align_pf_weight,
    ...
    a_loss_pf = (align_loss(embv1, embv2.detach()) +
