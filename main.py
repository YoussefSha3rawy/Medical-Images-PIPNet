...
def run_pipnet(args=None):
    ...
    trainloader, trainloader_pretraining, trainloader_normal, trainloader_normal_augment, projectloader, valloader, test_projectloader, classes = get_dataloaders(
        args, device)
    if len(classes) <= 20:
        if args.validation_size == 0.:
            print("Classes: ", valloader.dataset.class_to_idx, flush=True)
        else:
    ...
    with torch.no_grad():
        if args.state_dict_dir_net != '':
            ...
            try:
                ...
                pass
            if torch.mean(net.module._classification.weight).item(
            ) > 1.0 and torch.mean(net.module._classification.weight).item(
                ...
                print(
                    ...
                    flush=True)
                torch.nn.init.normal_(net.module._classification.weight,
                                      mean=1.0,
                                      std=0.1)
                ...
                print("Classification layer initialized with mean",
                      ...
                      flush=True)
                if args.bias:
                    torch.nn.init.constant_(net.module._classification.bias,
                                            val=0.)
    ...
    if net.module._num_classes == 2:
        ...
        log.create_log('log_epoch_overview', 'epoch', 'test_top1_acc',
                       'test_f1', 'almost_sim_nonzeros',
                       'local_size_all_classes', 'almost_nonzeros_pooled',
                       'num_nonzero_prototypes', 'mean_train_acc',
        ...
        log.create_log('log_epoch_overview', 'epoch', 'test_top1_acc',
                       'test_top5_acc', 'almost_sim_nonzeros',
                       'local_size_all_classes', 'almost_nonzeros_pooled',
                       'num_nonzero_prototypes', 'mean_train_acc',
                       'mean_train_loss_during_epoch')
    ...
    for epoch in range(1, args.epochs_pretrain + 1):
        ...
        log.log_values('log_epoch_overview', epoch, "n.a.", "n.a.", "n.a.",
                       "n.a.", "n.a.", "n.a.", "n.a.", train_info['loss'])
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch <= epochs_to_finetune and (args.epochs_pretrain > 0
        ...
        if (epoch == args.epochs or epoch % 30 == 0) and args.epochs > 1:
            ...
            with torch.no_grad():
                ...
                net.module._classification.weight.copy_(
                    torch.clamp(net.module._classification.weight.data - 0.001,
                                min=0.))
        ...
        if eval_info['val_accuracy'] > best_acc:
            best_acc = eval_info['val_accuracy']
            net.eval()
            torch.save(
                {
                    'epoch':
    ...
    topks = visualize_topk(net, projectloader, len(classes), device,
                           'visualised_prototypes_topk', args)
    ...
    if topks:
        for prot in topks.keys():
            ...
            for (i_id, score) in topks[prot]:
                if score > 0.1:
                    found = True
            if not found:
                torch.nn.init.zeros_(net.module._classification.weight[:,
                                                                       prot])
        ...
        print(
            ...
            flush=True)
        eval_info = eval_pipnet(net, valloader, "notused" + str(args.epochs),
                                device, log)
        log.log_values('log_epoch_overview', "notused" + str(args.epochs),
                       eval_info['top1_accuracy'], eval_info['top5_accuracy'],
                       eval_info['almost_sim_nonzeros'],
                       eval_info['local_size_all_classes'],
                       eval_info['almost_nonzeros'],
    ...
    for c in range(net.module._classification.weight.shape[0]):
        ...
        if args.validation_size == 0.:
            print("Class",
                  c,
                  "(",
                  list(valloader.dataset.class_to_idx.keys())[list(
                      valloader.dataset.class_to_idx.values()).index(c)],
                  "):",
                  "has",
                  len(relevant_ps),
                  "relevant prototypes: ",
    ...
    if args.dataset == 'CUB-200-2011':
        ...
        project_path = os.path.split(
            os.path.split(projectset_img0_path)[0])[0].split("dataset")[0]
        ...
        csvfile_topk = get_topk_cub(net, projectloader, 10,
                                    'train_' + str(epoch), device, args)
        eval_prototypes_cub_parts_csv(csvfile_topk, parts_loc_path,
                                      parts_name_path, imgs_id_path,
                                      'train_topk_' + str(epoch), args, log)

        csvfile_all = get_proto_patches_cub(net,
                                            projectloader,
        ...
        eval_prototypes_cub_parts_csv(
            ...
            'train_all_thres' + str(cubthreshold) + '_' + str(epoch), args,
            log)

        print("\n\nEvaluating cub prototypes for test set", flush=True)
        csvfile_topk = get_topk_cub(net, test_projectloader, 10,
                                    'test_' + str(epoch), device, args)
        eval_prototypes_cub_parts_csv(csvfile_topk, parts_loc_path,
                                      parts_name_path, imgs_id_path,
    ...
    visualize(net, projectloader, len(classes), device,
    ...
    if args.extra_test_image_folder != '':
        if os.path.exists(args.extra_test_image_folder):
            vis_pred_experiments(net, args.extra_test_image_folder, classes,
    ...
    #     _, _, _, class_thresholds = get_thresholds(net, valloader, epoch, device, percent, log)
    #     print("Thresholds:", class_thresholds, flush=True)
    #     # Evaluate with in-distribution data
    #     id_fraction = eval_ood(net, valloader, epoch, device, class_thresholds)
