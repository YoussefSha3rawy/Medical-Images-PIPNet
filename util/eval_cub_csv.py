...
def eval_prototypes_cub_parts_csv(csvfile, parts_loc_path, parts_name_path,
    ...
    with open(csvfile, newline='') as f:
        ...
        for (prototype, imgname, h_min_224, h_max_224, w_min_224,
             w_max_224) in filereader:
            ...
            h_min_224, h_max_224, w_min_224, w_max_224 = float(
                h_min_224), float(h_max_224), float(w_min_224), float(
                    w_max_224)
    ...
    print("Number of prototypes in parts_presences: ",
    ...
    print("Number of part-related prototypes (purity>0.5): ",
          prototypes_part_related)

    print("Mean purity of prototypes (corresponding to purest part): ",
          np.mean(list(max_presence_purity.values())), "std: ",
          np.std(list(max_presence_purity.values())))
    print("Prototypes with highest-purity part (no contraints): ",
          max_presence_purity_part)
    print("Prototype with part that has most often overlap with prototype: ",
          part_most_present)

    log.log_values(
        'log_epoch_overview', "p_cub_" + str(epoch),
        ...
        "std purity", "# prototypes in csv",
        "#part-related prototypes (purity > 0.5)", "", "")

    log.log_values('log_epoch_overview', "p_cub_" + str(epoch),
...
def get_proto_patches_cub(net,
    ...
    csvfilepath = os.path.join(args.log_dir,
                               str(epoch) + '_pipnet_prototypes_cub_all.csv')
    columns = [
        "prototype", "img name", "h_min_224", "h_max_224", "w_min_224",
        "w_max_224"
    ]
    with open(csvfilepath, "w", newline='') as csvfile:
        ...
        img_iter = tqdm(enumerate(range(len(imgs))),
                        total=len(imgs),
                        mininterval=50.,
                        ncols=0,
                    ...
                    if c_weight > 1e-5:  #ignore prototypes that are not relevant to any class
                        if pooled[prototype].item(
                            ...
                            h_coor_min, h_coor_max, w_coor_min, w_coor_max = get_img_coordinates(
                                args.image_size, pfs.shape, patchsize, skip,
...
def get_topk_cub(net, projectloader, k, epoch, device, args):
    ...
    too_small = set()
    protoype_iter = tqdm(enumerate(scores_per_prototype.keys()),
    ...
    with open(csvfilepath, "w", newline='') as csvfile:
        ...
        for _, prototype in protoype_iter:
            df = pd.DataFrame(scores_per_prototype[prototype],
                              columns=['img_id', 'scores'])
            ...
            for index, row in topk.iterrows():
                ...
                with torch.no_grad():
                    ...
                    location_h, location_h_idx = torch.max(
                        pfs[prototype, :, :], dim=0)
                    _, location_w_idx = torch.max(location_h, dim=0)
                    location = (location_h_idx[location_w_idx].item(),
                                location_w_idx.item())
                    h_coor_min, h_coor_max, w_coor_min, w_coor_max = get_img_coordinates(
                        args.image_size, pfs.shape, patchsize, skip,
                        location[0], location[1])
                    proto_img_coordinates.append([
                        prototype, imgname, h_coor_min, h_coor_max, w_coor_min,
                        w_coor_max
                    ])
