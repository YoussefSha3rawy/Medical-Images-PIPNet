...
def visualize_topk(net,
                   ...
                   k=10):
    print("Visualizing prototypes for topk...")
    ...
    for p in range(net._num_prototypes):
        if saved[p] > 0:
            ...
            text = "P " + str(p)
            txtimage = Image.new(
                "RGB", (img_tensor_patch.shape[1], img_tensor_patch.shape[2]),
                (0, 0, 0))
            draw = D.Draw(txtimage)
            draw.text((img_tensor_patch.shape[0] // 2,
            ...
            try:
                grid = torchvision.utils.make_grid(tensors_per_prototype[p],
                                                   nrow=k + 1,
                                                   padding=1)
    ...
    if len(all_tensors) > 0:
        ...
        torchvision.utils.save_image(grid,
                                     os.path.join(dir, "grid_topk_all.png"))
    else:
        print("Pretrained prototypes not visualized. Try to pretrain longer.")
    return topks


def visualize(net, projectloader, num_classes, device, foldername,
              args: argparse.Namespace):
            ...
            if c_weight > 0:
                ...
                if found_max > 0.5:
                    ...
                    h_coor_min, h_coor_max, w_coor_min, w_coor_max = get_img_coordinates(
                        ...
                        h_idx, w_idx)
                    img_tensor_patch = img_tensor[0, :, h_coor_min:h_coor_max,
                                                  w_coor_min:w_coor_max]
                    saved[p] += 1
                    tensors_per_prototype[p].append(
                        (img_tensor_patch, found_max))

                    save_path = os.path.join(dir, "prototype_%s") % str(p)
                    if not os.path.exists(save_path):
                        os.makedirs(save_path)
                    draw = D.Draw(image)
                    draw.rectangle([(w_coor_min, h_coor_min),
                                    (w_coor_max, h_coor_max)],
                                   outline='yellow',
                                   width=2)
                    image.save(
                        os.path.join(
                            save_path, 'p%s_%s_%s_%s_rect.png' %
    ...
    for p in range(net._num_prototypes):
        if saved[p] > 0:
            try:
                sorted_by_second = sorted(tensors_per_prototype[p],
                                          key=lambda tup: tup[1],
                                          reverse=True)
                sorted_ps = [i[0] for i in sorted_by_second]
                grid = torchvision.utils.make_grid(sorted_ps,
                                                   nrow=16,
                                                   padding=1)
                torchvision.utils.save_image(
...
def get_img_coordinates(img_size, softmaxes_shape, patchsize, skip, h_idx,
                        w_idx):
